...
def recommend_train(args, data, rating_np, show_loss):
    n_user, n_item, n_entity, n_relation = data[0], data[1], data[2], data[3]
    train_data, eval_data, test_data = data[4], data[5], data[6]
    kg = data[7]
    model = MKR(args, n_user, n_item, n_entity, n_relation)
    # 所有用户已经看过的图书
    all_user_record = get_user_record(rating_np, False)
    # 所有图书序号
    item_set = set(list(range(n_item)))
    # 所有用户序号
    all_user_list = list(range(n_user))
    rec_map = {}
        ...
        for step in range(args.n_epochs):
            ...
            if step % args.kge_interval == 0:
                ...
                while start < kg.shape[0]:
                    ...
                    if show_loss:
                        print(rmse)
        # 遍历所有用户，生成每个用户对每本图书的预测评分集合
        for user in all_user_list:
            test_item_list = list(item_set - all_user_record[user])
            item_score_map = dict()
            ...
            # args.num_recommend：给每个用户最大推荐个数
            for item in item_sorted[:args.num_recommend]:
                if user not in rec_map:
                    rec_map[user] = set()
                rec_map[user].add(item)
    for key in rec_map:
        rec_map[key] = list(rec_map[key])
    return rec_map
...
def train(args, data, show_loss, show_topk):
    ...
    user_list = list(set(train_record.keys()) & set(test_record.keys()))  # user_list：两组共同的 user
    ...
    with tf.Session() as sess:
        ...
        for step in range(args.n_epochs):
            ...
            # train_auc, train_acc, train_scores, train_labels, train_predictions = model.eval(
            #     sess, get_feed_dict_for_rs(model, train_data, 0, train_data.shape[0]))
            eval_auc, eval_acc = model.eval(sess, get_feed_dict_for_rs(model, eval_data, 0, eval_data.shape[0]))
            # eval_auc, eval_acc, eval_scores, eval_labels, eval_predictions = model.eval(
            #     sess, get_feed_dict_for_rs(model, eval_data, 0, eval_data.shape[0]))
            test_auc, test_acc = model.eval(sess, get_feed_dict_for_rs(model, test_data, 0, test_data.shape[0]))
            # test_auc, test_acc, test_scores, test_labels, test_predictions = model.eval(
            #     sess, get_feed_dict_for_rs(model, test_data, 0, test_data.shape[0]))

            # test_index, perd = model.get_scores(
            #     sess, get_feed_dict_for_rs(model, test_data, 0, test_data.shape[0]))
