...
class QuestionSet:
    ...
    def has_basic_movie_info_question(word_objects):
        ...
        #         e = u"?s :movieTitle '{movie}'." \
        #             u"?s :movieRating ?x." \
        #             u"?s :movieReleaseDate ?y.".format(movie=w.token)
        #         sparql = SPARQL_SELECT_TEM.format(
        #             prefix=SPARQL_PREXIX, select=select, expression=e)
        # return sparql
...
rules = [
    ...
    Rule(condition_num=4, condition=person_entity + Star(Any(), greedy=False) + compare + number_entity +
         ...
         action=QuestionSet.has_compare_question),
    ...
    Rule(condition_num=3, condition=person_entity + Star(Any(), greedy=False) + genre + Star(Any(),
                                                                                             greedy=False) + (
                                                movie | Star(Any(), greedy=False)),
         action=QuestionSet.has_specific_type_movie_question),
    Rule(condition_num=3, condition=person_entity + Star(Any(), greedy=False) + several + Star(Any(),
                                                                                               greedy=False) + (
                                                movie | Star(Any(), greedy=False)),
         action=QuestionSet.has_quantity_question),
    ...
    # 新建规则-Hugh
    Rule(condition_num=1, condition=(person_entity + Star(Any(), greedy=False)),
         action=QuestionSet.has_basic_person_info_question),
    Rule(condition_num=1, condition=(movie_entity + Star(Any(), greedy=False)),
    ...
    Rule(condition_num=3, condition=(person_entity + Star(Any(), greedy=False) + (when | where) + person_basic + Star(
        Any(), greedy=False)) |
                                    (person_entity + Star(Any(), greedy=False) + person_basic + Star(Any(),
                                                                                                     greedy=False)),
         action=QuestionSet.has_basic_person_info_question),
...
person_basic_keyword_rules = [
    KeywordRule(
        condition=(person_entity + Star(Any(), greedy=False) + where + birth_place + Star(Any(), greedy=False)) | (
                    person_entity +
                    Star(Any(), greedy=False) + birth_place + Star(Any(), greedy=False)),
    ...
    # 新建规则-Hugh
    KeywordRule(condition=person_entity + Star(Any(), greedy=False),
...
movie_basic_keyword_rules = [
    ...
    # 新建规则-Hugh
    KeywordRule(condition=movie_entity + Star(Any(), greedy=False),
